...
export default function Item({ name, desc, img, time, id, fav, changefavourite, onClick, ...props }) {


    return (
        <div className={container} onClick={() => onClick(id)} {...props}>
            <div className={text}>
                <div className={description}>
                    <Text onClick={(e) => e.stopPropagation()} size={'mega'}>{name}</Text>
                    <div style={{ height: 10 }} />
                ...
                <div className={tags}>
                    <IconTag icon={fav ? IconStarFilled : IconStar} variants={'yellow'} onClick={(e) => { e.stopPropagation(); changefavourite(id, !fav) }} color={token('colors.yellow.0')}></IconTag>
            ...
            <div className={image}>
                <img className={css({ borderRadius: 10, width: 700, objectFit: 'cover', maxHeight: '300px' })} src={img || 'https://static1.cbrimages.com/wordpress/wp-content/uploads/2022/10/Bocchi-The-Rock!-Episode-4-Hitori-Gotou-Loses-Her-Mind-Crunchyroll.jpg'} />
            </div>
        </div >
    )
}
