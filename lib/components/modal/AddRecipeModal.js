...
export default function AddRecipeModal({ opened, onClose: close, id, setId }) {
    ...
    const onAdd = () => {
        dispatch({ type: id ? 'edit' : 'add', id, values: { name, description, image, ingredients, instructions } });
            ...
            <div className={container} onClick={e => e.stopPropagation()}>
                ...
                <div className={css({ display: 'flex', gap: '10px' })}>
                    ...
                    <div className={css({ flex: 1 })}>
                        ...
                        <div className={css({ maxHeight: '200px', overflow: 'auto', display: 'flex', flexDirection: 'column', gap: '5px', marginBottom: '5px' })}>
                            {instructions.map((m, i) => <Input key={i} value={m} onChange={(c) => setInstructions((ing) => { const newArray = [...ing]; newArray[i] = c.target.value; console.log(newArray); return newArray; })} />)}

                        </div>
                        <Button theme={'white'} small onlyIcon icon={IconPlus} onClick={() => setInstructions((i) => [...i, ''])} />
                    </div>
                </div>
